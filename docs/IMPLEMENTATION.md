...
Projectile trails use one preallocated six-point ring buffer per pellet slot. Player shots leave a roughly 35-pixel lime or orange tapered trail; turret shots use a shorter, dimmer four-point purple treatment. Seven-pixel distance sampling keeps the result stable across frame rates, while deterministic perpendicular warble and one computed satellite droplet add organic motion without particle nodes, shaders, textures, or runtime allocations. Ricochets retain their directional bend. Reduced Motion keeps the static taper but disables trail displacement, bead drift, and projectile-radius warble.
...
- Fixed pools: 37 germs (35 regular, one elite, one shared milestone-boss slot), 80 debris, 240 projectiles, 3 turrets, 48 mines, 4 pickup/aura slots, 3 boon choices, and 64 goo patches
