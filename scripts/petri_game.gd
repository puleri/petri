...
func _player_fire_interval() -> float:
	...
	var catalyst_multiplier := ItemData.catalyst_shots_per_second(catalyst_level) / ItemData.catalyst_shots_per_second(0)
	return 1.0 / maxf(base_weapon_fire_rate * catalyst_multiplier, 0.001)
...
func _update_germs(delta: float) -> void:
	...
	for i in germs.size():
		...
		if _is_boss_tier(int(g.tier)):
			var previous_position := Vector2(g.pos)
			g = _update_boss_movement(g, simulation_delta)
			g.pos = previous_position + (Vector2(g.pos) - previous_position) * local_speed_mult
		else:
			...
			g.pos += Vector2(g.vel) * speed_mult * simulation_delta * local_speed_mult
...
func _trigger_antibody_shell() -> bool:
	...
	popups.append({"pos": player_pos, "text": "SHELL BURST", "life": 1.0, "duration": 1.0, "item_type": ItemData.ItemType.ANTIBODY_SHELL, "occlusion_opacity": 1.0})
	audio.play_sfx("impact")
	if not bool(saved.reduced_motion):
...
func _draw_how_to() -> void:
	...
	_draw_text_centered("Items add attacks, control, or protection. Three unprotected hits end the run.", Vector2(viewport_size.x * 0.5, panel.end.y - 30.0), 15, ORANGE_HOT)
